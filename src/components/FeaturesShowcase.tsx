export default function ServicesLayout() {
  ...
  return <div className="min-h-screen w-full bg-black text-white">
      ...
      <section className="mx-auto max-w-6xl px-6 pt-20 pb-10">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-center text-white/70">Live KPI boards, forecasts, and next best actions, so teams act, not guess.</p>
      </section>
  ...
  return <div className={`grid items-center gap-8 md:grid-cols-2 ${reverse ? "md:[&>div:first-child]:order-2" : ""}`}>
      ...
      <div>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <a href="#" className="rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-fuchsia-500/20 hover:opacity-90">
            {service.ctas[0]}
          </a>
          <a href="#" className="rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-medium text-white/90 hover:bg-white/10">
            {service.ctas[1]}
          </a>
        </div>
      </div>
